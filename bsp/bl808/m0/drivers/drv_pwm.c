...
static rt_err_t bl808_pwm_set(rt_uint8_t channel, struct rt_pwm_configuration *configuration)
{    
...
static rt_err_t bl808_pwm_control(struct rt_device_pwm *device, int cmd, void *arg)
{
    ...
    case PWM_CMD_GET:
        return bl808_pwm_get(channel, configuration);
    default:
        return RT_EINVAL;
    }
}
...
static struct rt_device_pwm bl808_pwm;
...
int bl808_hw_pwm_init(void)
{
    ...
    bflb_pwm_v2_start(pwm);
        ...
        rt_device_pwm_register(&bl808_pwm, "pwm", &drv_ops, 0);
    #endif
    return RT_EOK;
}
INIT_DEVICE_EXPORT(bl808_hw_pwm_init);
