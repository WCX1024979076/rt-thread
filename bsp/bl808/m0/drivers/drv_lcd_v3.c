...
int lcd_draw_area(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color)
{
    ...
    uint16_t w = x2 - x1 + 1;
    uint16_t h = y2 - y1 + 1;
    uint16_t *buff = rt_malloc(w * h * 2);
    if (!buff) {
        res = -1;
        printf("Malloc failed! abaaba\r\n");
        goto _exit;
    }
...
int lcd_draw_is_busy(void) { return private.is_busy; }
...
{
    return lcd_draw_area(0, 0, private.width - 1, private.hight - 1, color);
}